  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      if (!decodedToken || !decodedToken.uid) {
        throw new UnauthorizedException('Token inválido ou UID não encontrado.');
      }
      ...
      if (!mongoUser) {
        ...
        throw new UnauthorizedException('Usuário não encontrado no sistema após sincronização com Firebase.');
      }
      ...
      throw new UnauthorizedException(error.message || 'Falha na autenticação via Firebase.');
    }
  }
}